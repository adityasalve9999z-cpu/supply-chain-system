...
public sealed class CatalogController(SupplyChainDbContext db) : ControllerBase
{
    ...
    [HttpPost("categories"), Authorize(Roles = "Admin")] public async Task<Category> CreateCategory(CategoryRequest request, CancellationToken ct) { var c = new Category { Name = request.Name, Description = request.Description ?? "" }; db.Categories.Add(c); await db.SaveChangesAsync(ct); return c; }
